...
- **Backward (exact-pin only)** — for each package P in the set, scan every other workspace package S. If S's `peerDependencies` exact-pins P via `workspace:*` (which pnpm publishes as exact `X.Y.Z`), include S too. Prevents the dual failure mode where S's last published tarball still carries the **old** exact peer pin → downstream installs both new P and a duplicate **old** P to satisfy S's stale peer (v0.26.3 anchor — Lexical error #8 from two `@haklex/rich-editor` copies on disk → two `lexical` runtimes → node-class mismatch).

Note the asymmetry: forward walk follows any `workspace:` prefix (`*`, `^`, `~`, explicit), because pnpm publishes them all as **some** concrete version and the consumer must be able to resolve them. Backward walk fires **only** on `workspace:*` peers, because `workspace:^` peers publish as `^X.Y.Z` and tolerate sibling minor/patch bumps without republish. Bumps that cross the caret floor (i.e. `major`) still force `MODE=full` via Phase 2's auto-fallback, so backward walk does not need to handle them.
...
case "$MODE" in
  ...
  incremental)
    ...
    while [ "$changed" -eq 1 ]; do
      ...
      # Backward: any sibling whose peerDeps exact-pin (workspace:*) a member of IN_SET
...
| Equating `PUBLISH_SET` with `CHANGED_PKGS` in `incremental` mode           | This repo uses `workspace:*` — pnpm publishes those as exact versions. `PUBLISH_SET` must be the **bidirectional** closure of `CHANGED_PKGS`: forward over `dependencies + peerDependencies + optionalDependencies`, backward over sibling `peerDependencies` exact-pins (`workspace:*`). Forward-only misses the reverse failure mode — sibling tarballs with stale exact peer pins force downstream to install a duplicate old copy of the bumped package. Real-world hits: v0.21.0 (forward), v0.26.3 (backward) |
