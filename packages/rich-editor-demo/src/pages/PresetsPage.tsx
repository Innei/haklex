...
function getInitialState() {
  ...
  return {
    selectedKey:
      preset && validPresetKeys.has(preset) ? preset : presets[0].key,
    variant:
      variant && validVariants.has(variant)
        ? variant
        : ('article' as RichEditorVariant),
...
export function PresetsPage() {
  ...
  const jsonData =
    mode === 'edit' ? (liveState ?? selected.data) : selected.data
