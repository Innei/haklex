...
function typedToParsed(typed: ExcalidrawSnapshot, apiUrl: string | undefined): ParsedData {
  switch (typed.type) {
    case 'inline': {
...
export function useExcalidrawData(data: string | ExcalidrawSnapshot | null): ExcalidrawDataState {
  ...
  const [remoteSnapshot, setRemoteSnapshot] = useState<Record<string, any> | undefined>();
