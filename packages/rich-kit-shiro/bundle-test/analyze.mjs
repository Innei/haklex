...
for (const [pattern, label] of editorPatterns) {
  ...
  if (matches) {
    ...
    const ctx = content.substring(Math.max(0, idx - 30), Math.min(content.length, idx + 50)).replaceAll('\n', ' ')
...
for (const d of dynImports) {
  ...
  if (lower.includes('edit') || lower.includes('excalidraw') || lower.includes('codemirror') || lower.includes('cm-')) {
    console.log(`  ${  d}`)
  }
}
